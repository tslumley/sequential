...
Analyze<-function(object,...) UseMethod("Analyze", object)

## FIXME: factor out the updating from the text and graphics output.

Analyze.PoissonSetup<- function(object,test,mu0,events,AlphaSpend="n")
{


title<- object$titlecheck
...
if(AlphaSpend>alpha&AlphaSpend!="n"){stop(c("'AlphaSpend' must be smaller than or equal to ",alpha,"."),call. =FALSE)}
 ...
 alphaspend<- object$alphaspend #has the target alpha spending planed for future looks and settled until the (test-1)th look.
...
## Setting up the target alpha spending.
    ...
    if(SampleSize<=mu0+sum(inputSetUp[6,1:(test-1)])){
        current_alpha<- alpha
    } else{
...
result2<- result[2:(test+1),]
    ...
    rval<-list(alphaspend=alphaspend,inputSetUp=inputSetUp, result2=result2,p=p,title=title, SampleSize=SampleSize,
               test=test,observed=observed,critical_values=critical_values,xs=x,alpha=alpha)
    class(rval)<-c("PoissonSetupWithResults","PoissonSetup","Setup")
...
summary.PoissonSetupWithResults<-function(object, ...){

 message(c("                                ",object$titlecheck),domain = NULL, appendLF = TRUE)
## message("-------------------------------------------------------------------------------------------",domain = NULL, appendLF = TRUE)      
##           message(paste(c("=>    H0 was rejected on test"," ",reject,". ","No further sequential analyses are needed.")),domain = NULL, appendLF = TRUE)  
## message("-------------------------------------------------------------------------------------------",domain = NULL, appendLF = TRUE)
 options("width"=300)
 print(object$result2,right=TRUE,row.names=FALSE)
    with(object,{
        message("===========================================================================================",domain = NULL, appendLF = TRUE)
        message(c("Parameter settings: Sample size= ",SampleSize,", alpha= ",alpha,", mu0= ",mu0," and M= ",M,"."),domain = NULL, appendLF = TRUE)
        message(c("Analysis performed on ",date(),"."),domain = NULL, appendLF = TRUE)
...
plot.PoissonSetupWithResults<-function(x,...){
...
# Graphic 1
    ...
    with(x,{
        ...
        points(xs,critical_values,col="red",pch=20)
        lines(xs,critical_values,col="red",lty=2)
